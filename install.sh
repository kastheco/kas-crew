...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/kastheco/kas-crew/releases")
    if [ $? -ne 0 ]; then
...
extract_and_install() {
    ...
    mv "${tmp_dir}/kc${extension}" "$bin_dir/$INSTALL_NAME${extension}"
    rm -rf "$tmp_dir"
...
main() {
    ...
    RELEASE_URL="https://github.com/kastheco/kas-crew/releases/download/v${VERSION}"
    ARCHIVE_NAME="kas-crew_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
